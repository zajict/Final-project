...
export const SingleUser = () => {
    ...
    useEffect(() => {
        fetch(`http://localhost:3333/api/candidates/?id=${id}`).then(response => response.json()).then(data => setUser(data))
    }, [id]);
    ...
    useEffect(() => {
        fetch(`http://localhost:3333/api/reports?candidateId=${candidateId}`).then(response => response.json()).then(data => setReports(data))
    }, [id]);
                ...
                <tbody> {
                    reports.map(report => (<>
                        ...
                        <tr>
                            ...
                            <td><FaEye onClick={
                                    () => modalHandler(report.candidateId)
                                }/></td>
                        </tr>
                    </>))
                } </tbody>
